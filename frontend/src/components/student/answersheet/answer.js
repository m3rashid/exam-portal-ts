...
class Answer extends React.Component {
  ...
  OpenModel = (qid) => {
    this.setState({ ActiveQuestionId: qid, Mvisible: true });
  };
    ...
    Post({
      ...
      .then((response) => {
        if (response.data.success) {
          this.setState({ qdetails: response.data.data[0] });
      ...
      return (
        <div>
          ...
          <div style={{ marginLeft: 10 }}>
            <strong>Explanation</strong> : <span>{Optiondata.explanation}</span>
          </div>
          <div style={{ marginLeft: 10 }}>
            <strong>Level</strong> : <span>{Optiondata.level}</span>
            <br />
            <strong>Difficulty</strong> : <span>{Optiondata.difficulty}</span>
          </div>
        </div>
      );
    } else {
